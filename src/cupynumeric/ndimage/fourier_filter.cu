...
static __global__ void __launch_bounds__(THREADS_PER_BLOCK, MIN_CTAS_PER_SM)
  fourier_filter_kernel(VAL* output,
...
template <typename VAL, int DIM>
struct NdimageFourierFilterImplBody<VariantKind::GPU, VAL, DIM> {
  TaskContext context;
                  ...
                  const NdimageFourierFilterParams params) const
  {
    ...
    fourier_filter_kernel<<<num_blocks, THREADS_PER_BLOCK, 0, stream>>>(output_ptr,
...
/*static*/ void NdimageFourierFilterTask::gpu_variant(TaskContext context)
{
  ndimage_fourier_filter_template<VariantKind::GPU>(context);
}
