...
struct TrsmImpl {
  TaskContext context;
  explicit TrsmImpl(TaskContext context) : context(context) {}
  ...
  void operator()(legate::PhysicalStore lhs_array, legate::PhysicalStore rhs_array) const
  {
    ...
    TrsmImplBody<KIND, CODE>{context}(lhs, rhs, m, n);
  }
...
static void trsm_template(TaskContext& context)
{
  ...
  type_dispatch(lhs.type().code(), TrsmImpl<KIND>{context}, lhs, rhs);
}
