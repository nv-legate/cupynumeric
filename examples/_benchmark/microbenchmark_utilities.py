...
class MicrobenchmarkCall:
    """A concrete microbenchmark call generated by suite-local sizing."""
...
def _group_microbenchmark_calls(
    calls: Iterable[MicrobenchmarkCall],
) -> list[tuple[Callable[..., Any], list[MicrobenchmarkCall]]]:
    groups: dict[
        tuple[int, str], tuple[Callable[..., Any], list[MicrobenchmarkCall]]
    ] = {}
    for call in calls:
        key = (id(call.function), call.name)
        if key not in groups:
            groups[key] = (call.function, [])
...
class MicrobenchmarkSuite(BenchmarkHarness):
    ...
    def run_timed_calls(
        self, calls: Iterable[MicrobenchmarkCall], **kwargs: Any
    ) -> None:
        for function, call_group in _group_microbenchmark_calls(calls):
            info = get_benchmark_info(function).replace(
                name=call_group[0].name
            )
            self.run_timed_with_generator(
                info, function, (call.args for call in call_group), **kwargs
            )
    ...
    def print_size_resolution(
        ...
        self.print_panel(lines, title=f"Memory Size Target: {self.name}")
