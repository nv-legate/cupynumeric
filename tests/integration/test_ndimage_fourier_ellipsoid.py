...
SHAPES = [(1,), (7,), (8,), (5, 6), (6, 5), (4, 5, 6)]

SIZES = [0.0, 0.5, 2.0, (0.5, 1.0), (0.0, 1.0, 2.0)]
...
def test_fourier_ellipsoid_complex_fft_all_axes(dtype, shape):
    ndim = len(shape)
    ...
    for size in _valid_sizes(ndim):
        expected = scipy_ndimage.fourier_ellipsoid(image_tf_np, size, n=-1)
        actual = num.ndimage.fourier_ellipsoid(image_tf_num, size, n=-1)
...
@pytest.mark.parametrize("shape", SHAPES)
def test_fourier_ellipsoid_real_fft_axis_modes(dtype, shape):
    ndim = len(shape)
    image_tf_np = np.fft.fftn(_make_input(shape, dtype)).astype(dtype)
    image_tf_num = num.array(image_tf_np)

    for axis, n in product(range(-ndim, ndim), REAL_FFT_N):
        expected = scipy_ndimage.fourier_ellipsoid(
            image_tf_np, 2.0, n=n, axis=axis
        )
        actual = num.ndimage.fourier_ellipsoid(
            image_tf_num, 2.0, n=n, axis=axis
        )
        if n == 0:
            assert np.isnan(actual).any() and np.isnan(expected).any()
        else:
            _assert_allclose(actual, expected, n=n)


@pytest.mark.parametrize("shape", [(7,), (5, 6), (4, 5, 6)])
def test_fourier_ellipsoid_matches_rfft_convention(shape):
...
def test_fourier_ellipsoid_singleton_dimensions(shape):
    image_tf_np = _make_input(shape, np.complex128)
    image_tf_num = num.array(image_tf_np)

    size = tuple(float(i + 1) for i in range(len(shape)))
    expected = scipy_ndimage.fourier_ellipsoid(image_tf_np, size)
    actual = num.ndimage.fourier_ellipsoid(image_tf_num, size)
...
def test_fourier_ellipsoid_corner_sizes(shape, size):
...
def test_fourier_ellipsoid_output_argument():
    image_tf_np = _make_input((5, 6), np.complex128)
    image_tf_num = num.array(image_tf_np)

    output = num.empty_like(image_tf_num)
    result = num.ndimage.fourier_ellipsoid(image_tf_num, 2.0, output=output)

    expected = scipy_ndimage.fourier_ellipsoid(image_tf_np, 2.0)
